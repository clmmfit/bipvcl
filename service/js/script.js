...
let Service = function () {
                ...
                if (data.status == true) {
                    ...
                    if (data.data_momo) {
                        ...
                        for (let momo of data.data_momo) {
                            ...
                            table_one += `<tr>`;
                            table_one += `<td>${momo.username} <span class="label label-success text-uppercase" onclick="Service.coppy('${momo.username}')"><i class="fa fa-clipboard" aria-hidden="true"></i></span> <span class="label label-success text-uppercase" onclick="Service.play('${momo.username}')"><i class="fa fa-play" aria-hidden="true"></i></span></td>`;
    ...
    reset_data = function () {
        ...
        if (sec % 2 == 0) {
            ...
            if (id_his > 0) {
                Service.noti_win(noti_win[id_his]);
...
"undefined" != typeof module && "undefined" != typeof module.exports && (module.exports = Service), $(document).ready(function () {
    Service.init()
    var pusher = new Pusher(pusher_key, {
        cluster: "ap1",
        authEndpoint: ServiceSettings.pusher.authEndpoint,
        channel_auth_endpoint: ServiceSettings.pusher.channel_auth_endpoint,
        crypt_master_key_base64: ServiceSettings.pusher.crypt_master_key_base64,
    });
});
    ...
    check_amount = function (partnerId) {
        ...
        initAjax({
            ...
            success: function (data_xhr) {
                if (data_xhr.status == true) {
                    Swal.fire({
                        title: 'Báº¡n Ä‘ang cĂ³ ' + Service.number_format(data_xhr.data.amount) + ' vnÄ‘. Nháº­p sá»‘ tiá»n muá»‘n rĂºt:',
                        input: 'number',
                        inputAttributes: {
                            autocapitalize: 'off'
                        },
...
$("#result_hu [name=partnerId]").on("input", function () {
    if (this.value.length >= 10 && this.value.length <= 11) {
        $("#msg_hu2").html('<button type="text" class="btn btn-success submit-join-hu" onclick="Service.joinhu()">Tham gia</button>');
    } else {
